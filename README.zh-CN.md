...
这个工程用于把两个 ONNX 策略模型合并到同一个 ONNX 文件中。核心思路是将不同onnx中包含完整模型信息的图并列之后封装到一个新的onnx中，子图间的合并使用onnx官方给出的条件分支语句实现。对于不同的子图的输入观测差异问题，在新的onnx下可以取并集，然后在选定子图后从onnx中取所需的部分做映射即可。针对模型切换时在真机中可能出现的关节突变问题，可以尝试在ROS的部署代码中做平滑处理。
